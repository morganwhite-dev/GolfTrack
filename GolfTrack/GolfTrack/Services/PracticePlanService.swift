...
enum PracticePlanService {
    ...
    private static func focusAreas(round: GolfRound, stats: RoundStats) -> [FocusArea] {
        var areas: [FocusArea] = []
        let reflection = round.reflection
        let signals = RoundAnalysisService.extractTextSignals(from: round)

        // --- Stats-based areas ---

        if stats.threePutts >= 2 || reflection?.puttingFeel == .poor || signals.wantsToImprovePutting {
        ...
        if shortMisses >= 3 || signals.wantsToImproveDistance {
        ...
        if (stats.holes.count > 0 && Double(poorContactHoles) / Double(stats.holes.count) >= 0.4) || signals.wantsToImproveContact {
        ...
        if reflection?.hadMentalMistakes == true || stats.blowUpHoles >= 2 || signals.wantsToImproveMental {
        ...
        // --- Text-signal-only areas (only added if not already captured above) ---

        func alreadyHas(_ label: String) -> Bool {
            areas.contains { $0.label.lowercased().contains(label.lowercased()) }
        }

        if signals.wantsToImproveWedge && !alreadyHas("wedge") && !alreadyHas("contact") {
            areas.append(FocusArea(label: "Wedge Game", drills: [
                drill("Wedge Distance Ladder", .wedges, "Hit 10 shots each to 50, 75, and 100 yards — focus on landing the ball on the correct distance, not just near the flag.", 15, skill: "Wedge distance control")
            ]))
        }

        if signals.wantsToImproveDriver && !alreadyHas("driver") && !alreadyHas("tee") && !alreadyHas("alignment") {
            areas.append(FocusArea(label: "Tee Shots", drills: [
                drill("Half-Driver Drill", .driver, "Hit 10 drives at 70% power, focusing on balance and a consistent finish. Add power only once contact is solid.", 15, skill: "Driver consistency")
            ]))
        }

        if signals.wantsToImproveShortGame && !alreadyHas("chip") && !alreadyHas("short game") {
            areas.append(FocusArea(label: "Short Game", drills: [
                drill("Landing Zone Chipping", .chipping, "Pick a specific landing spot 2-3 feet onto the green and chip to it — land the ball there, let it release.", 10, skill: "Chip landing accuracy")
            ]))
        }
    ...
    static func generatePlan(round: GolfRound, stats: RoundStats, advice: RoundAdvice) -> PracticePlan {
        let areas = focusAreas(round: round, stats: stats)
